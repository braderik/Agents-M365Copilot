...
export const procurementTools: Tool[] = [
  {
    ...
    description:
      "Search Dynamics 365 F&O purchase orders by number, vendor, status, date, and amount.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description:
      "Query purchase order lines across all POs in Dynamics 365 F&O.",
    inputSchema: {
      type: "object",
      properties: {
        poNumber: { type: "string" },
...
export async function handleProcurementTool(
  ...
  switch (name) {
    case "proc_query_purchase_orders": {
      ...
      if (a.poNumber) filters.push(`contains(PurchaseOrderNumber,'${a.poNumber}')`);
      if (a.vendorAccount) filters.push(`VendorAccountNumber eq '${a.vendorAccount}'`);
      if (a.status) filters.push(`PurchaseOrderStatus eq '${a.status}'`);
      if (a.dateFrom) filters.push(`DocumentDate ge ${a.dateFrom}`);
      if (a.dateTo) filters.push(`DocumentDate le ${a.dateTo}`);
      if (a.amountMin !== undefined) filters.push(`TotalInvoiceAmount ge ${a.amountMin}`);
    ...
    case "proc_get_purchase_order": {
      ...
      try {
        const key: Record<string, string> = { PurchaseOrderNumber: a.poNumber };
    ...
    case "proc_query_po_lines": {
      ...
      const filters: string[] = [];
      if (a.poNumber) filters.push(`PurchaseOrderNumber eq '${a.poNumber}'`);
      if (a.itemNumber) filters.push(`ItemNumber eq '${a.itemNumber}'`);
      if (a.status) filters.push(`PurchaseOrderLineStatus eq '${a.status}'`);
      if (a.deliveryDateFrom) filters.push(`ConfirmedDeliveryDate ge ${a.deliveryDateFrom}`);
      if (a.deliveryDateTo) filters.push(`ConfirmedDeliveryDate le ${a.deliveryDateTo}`);
      if (a.dataAreaId) filters.push(`DataAreaId eq '${a.dataAreaId}'`);
    ...
    case "proc_query_requisitions": {
      ...
      const filters: string[] = [];
      if (a.requisitionNumber) filters.push(`RequisitionNumber eq '${a.requisitionNumber}'`);
      if (a.status) filters.push(`RequisitionStatus eq '${a.status}'`);
      if (a.requesterName) filters.push(`contains(RequestingWorkerName,'${a.requesterName}')`);
      if (a.dateFrom) filters.push(`RequestDate ge ${a.dateFrom}`);
      if (a.dateTo) filters.push(`RequestDate le ${a.dateTo}`);
      if (a.dataAreaId) filters.push(`DataAreaId eq '${a.dataAreaId}'`);
    ...
    case "proc_query_rfqs": {
      ...
      const filters: string[] = [];
      if (a.rfqNumber) filters.push(`RFQNumber eq '${a.rfqNumber}'`);
      if (a.vendorAccount) filters.push(`VendorAccountNumber eq '${a.vendorAccount}'`);
      if (a.status) filters.push(`RFQStatus eq '${a.status}'`);
      if (a.dateFrom) filters.push(`RFQDate ge ${a.dateFrom}`);
      if (a.dateTo) filters.push(`RFQDate le ${a.dateTo}`);
      if (a.dataAreaId) filters.push(`DataAreaId eq '${a.dataAreaId}'`);
    ...
    case "proc_query_receipts": {
      ...
      const filters: string[] = [];
      if (a.poNumber) filters.push(`PurchaseOrderNumber eq '${a.poNumber}'`);
      if (a.receiptNumber) filters.push(`ProductReceiptNumber eq '${a.receiptNumber}'`);
      if (a.itemNumber) filters.push(`ItemNumber eq '${a.itemNumber}'`);
      if (a.dateFrom) filters.push(`ProductReceiptDate ge ${a.dateFrom}`);
      if (a.dateTo) filters.push(`ProductReceiptDate le ${a.dateTo}`);
      if (a.dataAreaId) filters.push(`DataAreaId eq '${a.dataAreaId}'`);
