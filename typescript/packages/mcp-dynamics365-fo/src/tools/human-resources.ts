...
export const humanResourcesTools: Tool[] = [
  {
...
export async function handleHumanResourcesTool(
  name: string,
  ...
  switch (name) {
    case "hr_query_workers": {
      ...
      if (a.personnelNumber) filters.push(`contains(PersonnelNumber,'${a.personnelNumber}')`);
      if (a.name) filters.push(`contains(Name,'${a.name}')`);
      if (a.workerType) filters.push(`WorkerType eq '${a.workerType}'`);
      if (a.departmentId) filters.push(`PrimaryDepartmentId eq '${a.departmentId}'`);
      if (a.companyId) filters.push(`CompanyId eq '${a.companyId}'`);
      if (a.isActive === true) filters.push(`EmploymentEndDate eq null or EmploymentEndDate ge ${new Date().toISOString().split("T")[0]}`);
    ...
    case "hr_query_departments": {
      ...
      const filters: string[] = [];
      if (a.departmentNumber) filters.push(`DepartmentNumber eq '${a.departmentNumber}'`);
      if (a.name) filters.push(`contains(Name,'${a.name}')`);
    ...
    case "hr_query_positions": {
      ...
      const filters: string[] = [];
      if (a.positionId) filters.push(`PositionId eq '${a.positionId}'`);
      if (a.departmentId) filters.push(`DepartmentNumber eq '${a.departmentId}'`);
      if (a.jobId) filters.push(`JobId eq '${a.jobId}'`);
      if (a.isVacant === true) filters.push(`WorkerAssignedPersonnelNumber eq null`);
    ...
    case "hr_query_jobs": {
      ...
      const filters: string[] = [];
      if (a.jobId) filters.push(`JobId eq '${a.jobId}'`);
      if (a.title) filters.push(`contains(JobDescription,'${a.title}')`);
    ...
    case "hr_query_leave_balances": {
      ...
      const filters: string[] = [];
      if (a.personnelNumber) filters.push(`PersonnelNumber eq '${a.personnelNumber}'`);
      if (a.leaveType) filters.push(`LeaveTypeId eq '${a.leaveType}'`);
      if (a.companyId) filters.push(`DataAreaId eq '${a.companyId}'`);
    ...
    case "hr_query_compensation": {
      ...
      const filters: string[] = [];
      if (a.personnelNumber) filters.push(`PersonnelNumber eq '${a.personnelNumber}'`);
      if (a.planId) filters.push(`CompensationPlanId eq '${a.planId}'`);
      if (a.companyId) filters.push(`DataAreaId eq '${a.companyId}'`);
