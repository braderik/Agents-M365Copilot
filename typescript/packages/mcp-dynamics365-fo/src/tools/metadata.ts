...
const SearchEntitiesSchema = z.object({
  pattern: z.string().default("").describe(
    "Search pattern (partial name match). Leave empty to list all."
  ),
  category: z.string().optional().describe(
    "Entity category filter: Master, Transaction, Reference, Document, Configuration, Parameters"
  ),
...
export const metadataTools: Tool[] = [
  {
    ...
    description:
      "Search and discover Dynamics 365 F&O OData entity sets by name pattern. " +
  ...
  },
  {
    ...
    description:
      "Get the full OData schema for a Dynamics 365 F&O entity set, including " +
  ...
  },
  {
    ...
    description:
      "Search available OData actions and functions in the Dynamics 365 F&O instance. " +
  ...
  },
  {
    ...
    description:
      "Fetch a small sample of records from a Dynamics 365 F&O entity set (default 3 records). " +
  ...
  },
  {
    name: "d365_get_label",
    description:
      "Resolve a Dynamics 365 F&O label ID (in @SYS12345 format) to its human-readable text. " +
    ...
    case "d365_get_entity_schema": {
      ...
      try {
        ...
        return {
          type: "text",
          text: JSON.stringify(
            { source: "live", schema: schema ?? { entitySet: a.entitySet, note: "Schema not found" } },
            null,
            2,
          ),
        };
 ...
 */
async function parseEntitySchemaFromMetadata(
  client: DynamicsClient,
  entitySet: string,
  ...
  try {
    ...
    const entitySetMatch = xml.match(
      new RegExp(`<EntitySet[^>]+Name="${entitySet}"[^>]+EntityType="([^"]+)"`, "i"),
    );
    if (!entitySetMatch) return null;
    ...
    if (!entityTypeMatch) return null;
    ...
    // Extract properties
    const properties: { name: string; type: string; nullable: boolean; isKey: boolean }[] = [];
    ...
    const actionMatches = [...xml.matchAll(new RegExp(
      `<Action[^>]+Name="([^"]+)"[\\s\\S]*?<Parameter[^>]+Type="${entityTypeFull}"`,
      "gi",
    ))];
