...
export const salesTools = [
    {
        ...
        description: "Search Dynamics 365 F&O sales orders by number, customer, status, date range, and amount.",
        inputSchema: {
            type: "object",
            properties: {
...
// ─── Handlers ─────────────────────────────────────────────────────────────────
export async function handleSalesTool(name, args, client) {
    switch (name) {
        case "sales_query_orders": {
            const a = QuerySOSchema.parse(args);
            const filters = [];
            if (a.orderNumber)
                filters.push(`contains(SalesOrderNumber,'${a.orderNumber}')`);
            if (a.customerAccount)
                filters.push(`CustomerAccountNumber eq '${a.customerAccount}'`);
            if (a.status)
                filters.push(`SalesOrderStatus eq '${a.status}'`);
            if (a.dateFrom)
                filters.push(`CreatedDateTime ge ${a.dateFrom}`);
            if (a.dateTo)
                filters.push(`CreatedDateTime le ${a.dateTo}`);
            if (a.amountMin !== undefined)
                filters.push(`OrderTotalAmount ge ${a.amountMin}`);
            if (a.salesPersonnel)
        ...
        case "sales_query_order_lines": {
            ...
            const filters = [];
            if (a.orderNumber)
                filters.push(`SalesOrderNumber eq '${a.orderNumber}'`);
            if (a.itemNumber)
                filters.push(`ItemNumber eq '${a.itemNumber}'`);
            if (a.status)
                filters.push(`SalesOrderLineStatus eq '${a.status}'`);
            if (a.shipDateFrom)
                filters.push(`RequestedShippingDate ge ${a.shipDateFrom}`);
            if (a.shipDateTo)
                filters.push(`RequestedShippingDate le ${a.shipDateTo}`);
            if (a.dataAreaId)
                filters.push(`DataAreaId eq '${a.dataAreaId}'`);
        ...
        case "sales_query_quotations": {
            ...
            const filters = [];
            if (a.quotationId)
                filters.push(`QuotationId eq '${a.quotationId}'`);
            if (a.customerAccount)
                filters.push(`CustomerAccountNumber eq '${a.customerAccount}'`);
            if (a.status)
                filters.push(`QuotationStatus eq '${a.status}'`);
            if (a.expiryDateBefore)
        ...
        case "sales_query_price_lists": {
            ...
            const filters = [];
            if (a.customerAccount)
                filters.push(`AccountNumber eq '${a.customerAccount}'`);
            if (a.itemNumber)
                filters.push(`ItemNumber eq '${a.itemNumber}'`);
            if (a.priceGroupId)
                filters.push(`PriceDiscGroupCode eq '${a.priceGroupId}'`);
            if (a.dateFrom)
                filters.push(`FromDate ge ${a.dateFrom}`);
            if (a.dataAreaId)
                filters.push(`DataAreaId eq '${a.dataAreaId}'`);
