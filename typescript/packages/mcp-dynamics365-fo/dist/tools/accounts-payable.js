...
// ─── Tool definitions ─────────────────────────────────────────────────────────
export const accountsPayableTools = [
    {
    ...
    },
    {
        ...
        description: "Query vendor invoices in Dynamics 365 F&O. Supports filtering by vendor, status, date range, and due date.",
        inputSchema: {
            type: "object",
            properties: {
...
// ─── Handlers ─────────────────────────────────────────────────────────────────
export async function handleAccountsPayableTool(name, args, client) {
    switch (name) {
        case "ap_query_vendors": {
            const a = QueryVendorsSchema.parse(args);
            const filters = [];
            if (a.accountNumber)
                filters.push(`contains(VendorAccountNumber,'${a.accountNumber}')`);
            if (a.name)
                filters.push(`contains(VendorName,'${a.name}')`);
            if (a.groupId)
                filters.push(`VendorGroupId eq '${a.groupId}'`);
            if (a.onHold !== undefined)
        ...
        case "ap_query_invoices": {
            ...
            if (a.invoiceNumber)
                filters.push(`contains(InvoiceNumber,'${a.invoiceNumber}')`);
            if (a.status)
                filters.push(`InvoiceStatus eq '${a.status}'`);
            if (a.dateFrom)
                filters.push(`InvoiceDate ge ${a.dateFrom}`);
            if (a.dateTo)
                filters.push(`InvoiceDate le ${a.dateTo}`);
            if (a.dueDateFrom)
                filters.push(`DueDate ge ${a.dueDateFrom}`);
            if (a.dueDateTo)
        ...
        case "ap_query_payments": {
            ...
            if (a.vendorAccount)
                filters.push(`VendorAccountNumber eq '${a.vendorAccount}'`);
            if (a.dateFrom)
                filters.push(`PaymentDate ge ${a.dateFrom}`);
            if (a.dateTo)
                filters.push(`PaymentDate le ${a.dateTo}`);
            if (a.paymentMethod)
                filters.push(`PaymentMethod eq '${a.paymentMethod}'`);
            if (a.dataAreaId)
                filters.push(`DataAreaId eq '${a.dataAreaId}'`);
