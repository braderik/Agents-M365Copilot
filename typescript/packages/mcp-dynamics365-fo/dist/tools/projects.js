...
// ─── Handlers ─────────────────────────────────────────────────────────────────
export async function handleProjectsTool(name, args, client) {
    switch (name) {
        case "proj_query_projects": {
            const a = QueryProjectsSchema.parse(args);
            const filters = [];
            if (a.projectId)
                filters.push(`contains(ProjectId,'${a.projectId}')`);
            if (a.name)
                filters.push(`contains(Name,'${a.name}')`);
            if (a.projectType)
                filters.push(`ProjectType eq '${a.projectType}'`);
            if (a.status)
                filters.push(`ProjectStatus eq '${a.status}'`);
            if (a.customerAccount)
                filters.push(`CustomerAccountNumber eq '${a.customerAccount}'`);
            if (a.managerId)
                filters.push(`ProjectManagerPersonnelNumber eq '${a.managerId}'`);
            if (a.dataAreaId)
        ...
        case "proj_get_project": {
            ...
            try {
                const key = { ProjectId: a.projectId };
                if (a.dataAreaId)
        ...
        case "proj_query_transactions": {
            ...
            const filters = [];
            if (a.projectId)
                filters.push(`ProjectId eq '${a.projectId}'`);
            if (a.transactionType)
                filters.push(`TransactionType eq '${a.transactionType}'`);
            if (a.dateFrom)
                filters.push(`TransactionDate ge ${a.dateFrom}`);
            if (a.dateTo)
                filters.push(`TransactionDate le ${a.dateTo}`);
            if (a.workerId)
                filters.push(`WorkerPersonnelNumber eq '${a.workerId}'`);
            if (a.categoryId)
                filters.push(`CategoryId eq '${a.categoryId}'`);
            if (a.dataAreaId)
                filters.push(`DataAreaId eq '${a.dataAreaId}'`);
        ...
        case "proj_query_timesheets": {
            ...
            if (a.workerId)
                filters.push(`WorkerPersonnelNumber eq '${a.workerId}'`);
            if (a.dateFrom)
                filters.push(`TimesheetDate ge ${a.dateFrom}`);
            if (a.dateTo)
                filters.push(`TimesheetDate le ${a.dateTo}`);
            if (a.status)
                filters.push(`ApprovalStatus eq '${a.status}'`);
            if (a.dataAreaId)
                filters.push(`DataAreaId eq '${a.dataAreaId}'`);
        ...
        case "proj_query_expenses": {
            ...
            if (a.workerId)
                filters.push(`WorkerPersonnelNumber eq '${a.workerId}'`);
            if (a.dateFrom)
                filters.push(`ExpenseDate ge ${a.dateFrom}`);
            if (a.dateTo)
                filters.push(`ExpenseDate le ${a.dateTo}`);
            if (a.status)
                filters.push(`ApprovalStatus eq '${a.status}'`);
            if (a.dataAreaId)
                filters.push(`DataAreaId eq '${a.dataAreaId}'`);
        ...
        case "proj_query_contracts": {
            ...
            const filters = [];
            if (a.contractId)
                filters.push(`ContractId eq '${a.contractId}'`);
            if (a.customerAccount)
                filters.push(`CustomerAccountNumber eq '${a.customerAccount}'`);
            if (a.dataAreaId)
                filters.push(`DataAreaId eq '${a.dataAreaId}'`);
        ...
        case "proj_query_wbs": {
            ...
            const filters = [`ProjectId eq '${a.projectId}'`];
            if (a.dataAreaId)
                filters.push(`DataAreaId eq '${a.dataAreaId}'`);
