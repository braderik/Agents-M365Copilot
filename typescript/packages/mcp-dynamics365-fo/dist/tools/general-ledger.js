...
// ─── Tool definitions ─────────────────────────────────────────────────────────
export const generalLedgerTools = [
    {
        name: "gl_query_accounts",
        description: "Search the Dynamics 365 F&O chart of accounts. Returns main account IDs, names, types, and status.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Query posted general ledger journal entries (vouchers) in Dynamics 365 F&O by date range, account, voucher or batch number.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Retrieve the trial balance from Dynamics 365 F&O, optionally filtered by period dates and account.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Query budget register entries in Dynamics 365 F&O by model, fiscal year, and account.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "List financial dimension attribute values in Dynamics 365 F&O (e.g. Department, CostCenter, Project).",
        inputSchema: {
            type: "object",
            properties: {
...
// ─── Handlers ─────────────────────────────────────────────────────────────────
export async function handleGeneralLedgerTool(name, args, client) {
    switch (name) {
        case "gl_query_accounts": {
            const a = QueryAccountsSchema.parse(args);
            const filters = [];
            if (a.accountId)
                filters.push(`contains(MainAccountId,'${a.accountId}')`);
            if (a.accountType)
                filters.push(`Type eq '${a.accountType}'`);
        ...
        case "gl_query_journal_entries": {
            ...
            const filters = [];
            if (a.voucherNumber)
                filters.push(`Voucher eq '${a.voucherNumber}'`);
            if (a.journalBatchNumber)
                filters.push(`JournalBatchNumber eq '${a.journalBatchNumber}'`);
            if (a.dateFrom)
                filters.push(`TransactionDate ge ${a.dateFrom}`);
            if (a.dateTo)
                filters.push(`TransactionDate le ${a.dateTo}`);
            if (a.accountId)
                filters.push(`AccountDisplayValue eq '${a.accountId}'`);
            if (a.dataAreaId)
        ...
        case "gl_query_trial_balance": {
            ...
            const filters = [];
            if (a.periodStart)
                filters.push(`FromDate ge ${a.periodStart}`);
            if (a.periodEnd)
                filters.push(`ToDate le ${a.periodEnd}`);
            if (a.mainAccountId)
                filters.push(`MainAccountId eq '${a.mainAccountId}'`);
            if (a.dataAreaId)
                filters.push(`DataAreaId eq '${a.dataAreaId}'`);
        ...
        case "gl_query_budget": {
            ...
            const filters = [];
            if (a.budgetModelId)
                filters.push(`BudgetModelId eq '${a.budgetModelId}'`);
            if (a.mainAccountId)
                filters.push(`MainAccountId eq '${a.mainAccountId}'`);
            if (a.dataAreaId)
                filters.push(`DataAreaId eq '${a.dataAreaId}'`);
        ...
        case "gl_query_financial_dims": {
            ...
            const filters = [];
            if (a.dimensionName)
                filters.push(`Name eq '${a.dimensionName}'`);
        ...
        case "gl_query_fiscal_calendars": {
            ...
            const filters = [];
            if (a.calendarId)
                filters.push(`FiscalCalendar eq '${a.calendarId}'`);
