...
// ─── Tool definitions ─────────────────────────────────────────────────────────
export const accountsReceivableTools = [
    {
...
// ─── Handlers ─────────────────────────────────────────────────────────────────
export async function handleAccountsReceivableTool(name, args, client) {
    switch (name) {
        case "ar_query_customers": {
            const a = QueryCustomersSchema.parse(args);
            const filters = [];
            if (a.accountNumber)
                filters.push(`contains(CustomerAccountNumber,'${a.accountNumber}')`);
            if (a.name)
                filters.push(`contains(CustomerName,'${a.name}')`);
            if (a.groupId)
                filters.push(`CustomerGroupId eq '${a.groupId}'`);
            if (a.creditLimitMin !== undefined)
                filters.push(`CreditLimit ge ${a.creditLimitMin}`);
            if (a.creditLimitMax !== undefined)
        ...
        case "ar_query_invoices": {
            ...
            if (a.invoiceId)
                filters.push(`contains(InvoiceId,'${a.invoiceId}')`);
            if (a.status)
                filters.push(`DocumentStatus eq '${a.status}'`);
            if (a.dateFrom)
                filters.push(`InvoiceDate ge ${a.dateFrom}`);
            if (a.dateTo)
                filters.push(`InvoiceDate le ${a.dateTo}`);
        ...
        case "ar_query_open_transactions": {
            ...
            const filters = ["TransactionStatus eq 'Open'"];
            if (a.customerAccount)
                filters.push(`AccountDisplayValue eq '${a.customerAccount}'`);
            if (a.dueDateBefore)
        ...
        case "ar_query_collections": {
            ...
            if (a.customerAccount)
                filters.push(`CustomerAccountNumber eq '${a.customerAccount}'`);
            if (a.caseStatus)
                filters.push(`CaseStatus eq '${a.caseStatus}'`);
            if (a.dataAreaId)
                filters.push(`DataAreaId eq '${a.dataAreaId}'`);
