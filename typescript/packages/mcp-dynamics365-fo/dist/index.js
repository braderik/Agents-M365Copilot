...
function routeTool(name) {
    if (name.startsWith("gl_"))
        return handleGeneralLedgerTool;
    if (name.startsWith("ap_"))
        return handleAccountsPayableTool;
    if (name.startsWith("ar_"))
        return handleAccountsReceivableTool;
    if (name.startsWith("inv_"))
        return handleInventoryTool;
    if (name.startsWith("proc_"))
        return handleProcurementTool;
    if (name.startsWith("sales_"))
        return handleSalesTool;
    if (name.startsWith("hr_"))
        return handleHumanResourcesTool;
    if (name.startsWith("proj_"))
        return handleProjectsTool;
    if (name.startsWith("d365_download_") || name === "d365_download_report")
        return handleReportTool;
...
// ─── MCP Server ───────────────────────────────────────────────────────────────
const server = new Server({ name: "mcp-dynamics365-fo", version: "2.0.0" }, {
    capabilities: {
        tools: {},
...
server.setRequestHandler(ReadResourceRequestSchema, async (request) => readResource(request.params.uri, d365Client, d365Config.baseUrl));
...
async function main() {
    ...
    console.error(`[MCP D365] Dynamics 365 F&O MCP server v2.0.0 started\n` +
