...
export class DynamicsClient {
    ...
    async fetchAll(entitySet, options = {}) {
        ...
        let url = this.buildUrl(entitySet, pageOptions);
        while (url && records.length < FETCH_ALL_MAX_RECORDS) {
            // eslint-disable-next-line no-await-in-loop
            const axiosResp = await this.http.get(url);
            const page = axiosResp.data;
    ...
    async getByKey(entitySet, key, options = {}) {
        const keySegment = typeof key === "string"
     ...
     */
    async getMetadata() {
        const { data } = await this.http.get("$metadata", {
    ...
    async batchQuery(requests) {
        ...
        requests.forEach((req, i) => {
            ...
            parts.push(`--${batchBoundary}`, "Content-Type: application/http", "Content-Transfer-Encoding: binary", "", `GET ${url} HTTP/1.1`, "Accept: application/json", "", "");
            void i; // silence unused
        });
        parts.push(`--${batchBoundary}--`);
        const body = parts.join("\r\n");
        const token = await this.auth.getBearerToken();
        const { data: rawBatch } = await this.http.post("$batch", body, {
            headers: {
    ...
    buildUrl(entitySet, options) {
        ...
        if (options.dataAreaId)
            params.push(`$filter=DataAreaId eq '${options.dataAreaId}'`);
    ...
    parseBatchResponse(raw, requests) {
        ...
        requests.forEach((req, i) => {
            ...
            if (jsonStart !== -1 && jsonEnd !== -1) {
                try {
                    const parsed = JSON.parse(part.slice(jsonStart, jsonEnd + 1));
                    results.push({
    ...
    static formatError(error) {
        if (axios.isAxiosError(error)) {
            ...
            if (d365Msg)
                return `D365 Error ${d365Code ?? ""}: ${d365Msg}`;
